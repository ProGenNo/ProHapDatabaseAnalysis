...
df_disc = pd.read_csv(args.input_file_discoverable, header=0)
disc_ids = df_disc['variantID'].tolist().sort()
...
df_all['is_synonym'] = df_all['possible_conseq'].apply(lambda x: all(['synonymous' in c for c in x.split('|') if c != 'splice_variant']))
df_all['is_SAV'] = df_all['possible_conseq'].apply(lambda x: all(['SAV' in c for c in x.split('|') if c != 'splice_variant']))
df_all['is_stop_lost'] = df_all['possible_conseq'].apply(lambda x: all(['stop_lost' in c for c in x.split('|') if c != 'splice_variant']))
df_all['is_indel'] = df_all['possible_conseq'].apply(lambda x: all(['inframe_indel' in c for c in x.split('|') if c != 'splice_variant']))
df_all['is_frameshift'] = df_all['possible_conseq'].apply(lambda x: all(['frameshift' in c for c in x.split('|') if c != 'splice_variant']))
...
df_all['always_after_fs'] = df_all['possible_conseq'].apply(lambda x: all([('after_fs' in c) for c in x.split('|') if ('synonymous' not in c) and ('frameshift' not in c)]) and ('after_fs' in x))
...
print("Discoverable stop lost: %d (%.2f %%)" % len(df_all[df_all['is_discoverable'] & (df_all['stop_lost'] | df_all['is_stoplost_synon'])]), len(df_all[df_all['is_discoverable'] & (df_all['is_stop_lost'] | df_all['is_stoplost_synon'])]) / len(df_all[(df_all['is_stop_lost'] | df_all['is_stoplost_synon'])]) * 100)
